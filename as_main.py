...
_ATT_MAX_IMAGES = int(os.getenv("OZONATOR_MAX_IMAGES", "3"))

# лимит на ИСХОДНЫЙ файл (raw), чтобы не тащить совсем огромные фотки
_ATT_MAX_IMAGE_BYTES_EACH = int(os.getenv("OZONATOR_MAX_IMAGE_BYTES_EACH", str(15 * 1024 * 1024)))  # 15MB

# лимит на СУММУ уже СЖАТЫХ изображений, которые реально уйдут в LLM
_ATT_MAX_TOTAL_IMAGE_BYTES = int(os.getenv("OZONATOR_MAX_TOTAL_IMAGE_BYTES", str(8 * 1024 * 1024)))  # 8MB
...
def _prepare_image_for_vision(file_name: str, content_type: str, raw: bytes) -> tuple[bytes, str]:
    """Готовим изображение для vision: EXIF-поворот, ресайз и сжатие под лимит base64."""
    ...
    # Ограничение на длину base64-строки (символы). Делаю запас.
    target_b64 = int(os.getenv("OZONATOR_MAX_IMAGE_B64", "3500000") or 3500000)

    try:
        from PIL import Image, ImageOps  # type: ignore
    except Exception:
        # Без Pillow не можем гарантированно ужать изображение под лимиты провайдера.
        return raw, mime
    ...
    try:
        ...
        # Приводим к RGB, чтобы стабильно кодировать в JPEG
        if img.mode == "RGBA":
        ...
        max_dim = int(os.getenv("OZONATOR_MAX_IMAGE_DIM", "1600") or 1600)
        w, h = img.size
        if max(w, h) > max_dim:
        ...
        # 1) уменьшаем качество до 30
        while _b64_len(len(data)) > target_b64 and quality > 30:
            quality -= 8
            data = _encode(quality)

        # 2) если всё ещё велико — уменьшаем размеры
        shrink_steps = 0
        while _b64_len(len(data)) > target_b64 and shrink_steps < 8:
            w, h = img.size
            img = img.resize((max(1, int(w * 0.82)), max(1, int(h * 0.82))))
            data = _encode(max(30, quality))
            shrink_steps += 1
...
def _collect_attachments_for_llm(task_id: int) -> tuple[str, list[dict[str, Any]]]:
    ...
    for meta in files[:_ATT_MAX_FILES]:
        ...
        if ext in {"jpg", "jpeg", "png", "webp", "gif"} or (content_type or "").lower().startswith("image/"):
            ...
            if raw_size > _ATT_MAX_IMAGE_BYTES_EACH:
                blocks.append(
                    f"— {file_name}: (изображение пропущено — исходный файл слишком большой: {raw_size} bytes, лимит {_ATT_MAX_IMAGE_BYTES_EACH})"
                )
                continue
            ...
            # Если Pillow не установлен (или не смогли ужать) — часто это не пролезает в лимиты провайдера.
            def _b64_len(n: int) -> int:
                return ((n + 2) // 3) * 4

            max_b64 = int(os.getenv("OZONATOR_MAX_IMAGE_B64", "3500000") or 3500000)
            if _b64_len(processed_size) > max_b64:
                blocks.append(
                    f"— {file_name}: (изображение не удалось ужать под лимит vision; "
                    f"проверь, что Pillow установлен в AS и/или уменьши исходное фото)"
                )
                continue
            ...
            if processed_size < raw_size:
                blocks.append(f"— {file_name}: (изображение сжато {raw_size}->{processed_size} bytes и приложено к сообщению)")
            else:
...
def _force_feminine_voice(text: str) -> str:
    """Грубый пост‑процессор: исправляет типовые мужские формы в ответах Екатерины на женские."""
    ...
    # 1) Частые прилагательные/краткие формы в начале фразы (часто без "я")
    start_map = {
        "рад": "рада",
        "готов": "готова",
        "согласен": "согласна",
    ...
    for w_m, w_f in start_map.items():
        # начало строки/после переноса/после знаков
        text = re.sub(
            rf"(?im)(^|[\n\.!\?]\s*)(%s)(\b)" % re.escape(w_m),
            lambda m: m.group(1) + _case_like(m.group(2), w_f) + m.group(3),
            text,
        )

        # варианты "я рад/готов/согласен"
        text = re.sub(
            rf"(?im)\bя\s+(%s)(\b)" % re.escape(w_m),
            lambda m: "я " + _case_like(m.group(1), w_f) + m.group(2),
            text,
        )

    # 2) Частые глаголы прошедшего времени (я сделал/посмотрел/увидел…)
    past_map = {
        "сделал": "сделала",
        ...
        "сказал": "сказала",
        "запустил": "запустила",
        "перезапустил": "перезапустила",
        "заметил": "заметила",
        "попробовал": "попробовала",
    }

    for w_m, w_f in past_map.items():
        text = re.sub(
            rf"(?im)(\bя\s+)(%s)(\b)" % re.escape(w_m),
            lambda m: m.group(1) + _case_like(m.group(2), w_f) + m.group(3),
            text,
        )
        # иногда модель пишет без "я": "Сделал так-то…"
        text = re.sub(
            rf"(?im)(^|[\n\.!\?]\s*)(%s)(\b)" % re.escape(w_m),
            lambda m: m.group(1) + _case_like(m.group(2), w_f) + m.group(3),
            text,
...
def _strip_and_apply_self_profile_updates(text: str) -> str:
    ...
    cleaned = re.sub(r"\n{3,}", "\n\n", cleaned).strip()
    cleaned = _force_feminine_voice(cleaned)
    return cleaned
...
def _llm_answer(question_ru: str, payload: dict[str, Any] | None = None, *, image_parts: list[dict[str, Any]] | None = None) -> str:
    ...
    try:
        ...
        if image_parts:
            vision_hint = (question_ru + "\n\nПроверь, есть ли на изображении люди/лица (и сколько), и обязательно упомяни это в ответе. Если сомневаешься — так и скажи.")
            user_content = [{"type": "text", "text": vision_hint}, *image_parts]
